"""V2 fuzz of torch.nn.functional.max_pool2d: MPS vs CPU reference.

Mirrors the v2 driver pattern (5-seed reproducibility, denom_at_peak filter,
FILABLE vs RECALIBRATION vs OK rubric) but specialized for max_pool2d:

- Inputs are 4D (N, C, H, W). Shapes are sampled from 2D buckets and lifted
  to 4D by prepending small (N, C). Configs whose lifted (H, W) is smaller
  than the chosen kernel_size are skipped as `unsupported` (not errors).
- Kernel/stride/padding are sampled per config from a small grid.
- Stride categories from gpucheck.fuzzing.strides are applied to the 4D
  tensor; broadcast/gather categories may produce shapes incompatible with
  max_pool2d - those raise RuntimeError and are recorded as unsupported.

Halt rule: any unexpected exception aborts with exit 2 and a traceback,
per the "halt and report process error immediately" directive.
"""
...
# Use the active worktree for gpucheck import.
SRC = Path("/Users/cero/Code/gpucheck-worktrees/fuzz-max-pool/src")
...
RESULTS_MD = OUT_DIR / "RESULTS_max-pool.md"
SWARM_JSONL = OUT_DIR / "swarm.jsonl"

KERNEL = "torch.nn.functional.max_pool2d"
N_CONFIGS = 500
SEEDS: tuple[int, ...] = (0, 1, 2, 3, 4)
WALL_BUDGET_S = 8 * 60 - 60  # leave 60s for write-out and outer harness
MASTER_SEED = 0xCAFE

# ---------------------------------------------------------------------------
# Sampling space - 2D spatial shapes (H, W); we prepend (N, C) at runtime.
# ---------------------------------------------------------------------------

DEGENERATE_HW = [(1, 1), (1, 8), (8, 1), (2, 2), (3, 3)]
NON_TILE_HW = [
    (TILE_SIZES[0] - 1, TILE_SIZES[0] + 1),
    (TILE_SIZES[1] - 1, 16),
    (TILE_SIZES[1] + 3, 17),
    (TILE_SIZES[2] - 1, TILE_SIZES[2] + 1),
    (33, 17),
]
PRIME_HW = [(PRIMES[0], PRIMES[1]), (PRIMES[2], PRIMES[3]), (PRIMES[4], 16), (13, 7)]
POW2_BOUNDARY_HW = [(v, v) for v in POWER_OF_2_BOUNDARIES if v >= 4] + [(128, 129), (256, 255)]
LARGE_HW = [(64, 64), (128, 128), (256, 256), (512, 64)]
MIXED_HW = [(127, 16), (33, 65), (7, 128), (8, 9)]
...
# (N, C) to lift HW into 4D NCHW.
NC_CHOICES: list[tuple[int, int]] = [(1, 1), (1, 3), (2, 4), (1, 16), (4, 8)]

# (kernel_size, stride, padding) - all spatial-symmetric for simplicity.
POOL_CONFIGS: list[tuple[int, int, int]] = [
    (2, 2, 0),
    (3, 2, 1),
    (3, 1, 0),
    (4, 4, 0),
...
def _spatial_ok(shape4d: tuple[int, int, int, int], k: int, p: int) -> bool:
    """max_pool2d requires H + 2p >= k and W + 2p >= k, plus k > 2p."""
    _, _, h, w = shape4d
    if k <= 2 * p:  # torch refuses pad >= kernel/2
        return False
    return (h + 2 * p) >= k and (w + 2 * p) >= k
...
def main() -> int:
    ...
    for config_idx in range(N_CONFIGS):
        ...
        hw = rng.choice(SHAPE_BUCKETS[bucket])
        nc = rng.choice(NC_CHOICES)
        shape4d: tuple[int, int, int, int] = (nc[0], nc[1], hw[0], hw[1])
        dtype_name = rng.choice(DTYPE_NAMES)
        dtype = DTYPES_BY_NAME[dtype_name]
        stride_cat = rng.choice(STRIDE_CATEGORIES)
        k, s, p = rng.choice(POOL_CONFIGS)
        pool_tag = f"k{k}s{s}p{p}"

        per_bucket_counts[bucket] += 1
        per_dtype_counts[dtype_name] += 1
        per_stride_counts[stride_cat] += 1
        per_pool_counts[pool_tag] = per_pool_counts.get(pool_tag, 0) + 1

        if not _spatial_ok(shape4d, k, p):
            iters_unsupported += 1
            continue
            ...
            try:
                try:
                    x_cpu = fuzz_strides_for_category(
                        shape4d, dtype, stride_cat, device="cpu", seed=seed,
                    )
                    x_mps = fuzz_strides_for_category(
                        shape4d, dtype, stride_cat, device="mps", seed=seed,
                    )
                    ...
                    print(
                        ...
                        f"{bucket}/{dtype_name}/{stride_cat} shape={shape4d}: {msg}",
                        file=sys.stderr,
                    )
                    config_unsupported = True
                    break

                # max_pool2d expects 4D NCHW; broadcast/gather may have
                # surprising layouts but tensor rank is preserved by our
                # stride builders. Still wrap in try/except for safety.
                if x_cpu.numel() == 0:
                ...
                try:
                    y_mps = F.max_pool2d(x_mps, kernel_size=k, stride=s, padding=p)
                    ...
                    print(
                        ...
                        f"{bucket}/{dtype_name}/{stride_cat} shape={shape4d} "
                        f"pool={pool_tag}: {msg}",
                        file=sys.stderr,
                    )
                    config_unsupported = True
                ...
                try:
                    y_cpu = F.max_pool2d(x_cpu, kernel_size=k, stride=s, padding=p)
                except (RuntimeError, NotImplementedError, TypeError) as exc:
                    msg = str(exc).splitlines()[0][:160]
                    print(
        ...
        if len(diverg_seeds) >= 3:
            ...
            filable.append({
                ...
                "pool": {"kernel_size": k, "stride": s, "padding": p},
                "atol": atol,
                "rtol": rtol,
            ...
            print(
                ...
                f"shape={shape4d} pool={pool_tag} diverg_seeds={len(diverg_seeds)}/5 "
    ...
    summary = {
        ...
        "per_pool_config": per_pool_counts,
        "torch_version": torch.__version__,
        "mps_available": True,
...
def _write_markdown(path: Path, s: dict) -> None:
    ...
    lines.append("|---|---|")
    lines.append(f"| shape bucket (HW) | {s['per_shape_bucket']} |")
    lines.append(f"| dtype | {s['per_dtype']} |")
    lines.append(f"| stride category | {s['per_stride_category']} |")
    lines.append(f"| pool config (k/s/p) | {s['per_pool_config']} |")
    lines.append("")
    ...
    if not s["top_3_repros"]:
        ...
        for i, r in enumerate(s["top_3_repros"], 1):
            ...
            lines.append(f"- **pool:** kernel={r['pool']['kernel_size']}, "
                         f"stride={r['pool']['stride']}, padding={r['pool']['padding']}")
    ...
    lines.append("")
    lines.append("- Reference: `torch.nn.functional.max_pool2d` on CPU "
                 "(FP32-promoted comparison).")
    lines.append("- Inputs are 4D NCHW; (H, W) is sampled from a 6-bucket 2D corpus "
                 "and lifted by a small (N, C) prefix.")
    lines.append("- Pool configs: (k=2,s=2,p=0), (k=3,s=2,p=1), (k=3,s=1,p=0), "
                 "(k=4,s=4,p=0), (k=5,s=1,p=2), (k=2,s=1,p=0). Configs whose "
                 "spatial dims violate `H+2p >= k` are recorded as unsupported.")
    ...
    lines.append("- `denom_at_peak_rel` is `|y_cpu_fp32|` at the element where the "
                 ...
                 "near-zero-denominator artifacts (per v2 spec).")
